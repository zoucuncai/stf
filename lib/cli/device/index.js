...
module.exports.builder = function(yargs) {
  return yargs
    ...
    .option('screen-reset', {
      describe: 'On release, return to the default launcher (HOME x2 + intent) ' +
        'and reset screen rotation. Negate with --no-screen-reset.'
    , type: 'boolean'
    , default: true
    })
    .option('kill-apps-on-release', {
      describe: 'After returning to the launcher, stop all third-party app processes ' +
        '(am kill-all + force-stop for pm list packages -3). STF agent is never ' +
        'stopped. Requires --screen-reset. Negate with --no-kill-apps-on-release.'
    , type: 'boolean'
    , default: true
    })
...
module.exports.handler = function(argv) {
  ...
  , killAppsOnRelease: argv.killAppsOnRelease
  })
}
