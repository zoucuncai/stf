 ...
 */
module.exports.runExplorer = function(run, serials, push, ownerMsg) {
  var startedAt = now()
  var deviceChannels = {}

  // 提前加载设备 channel，用于运行结束后发送 UngroupMessage 释放设备
  db.run(r.table('devices').getAll(r.args(serials)))
    .then(function(cursor) { return cursor.toArray() })
    .then(function(devices) {
      devices.forEach(function(device) {
        if (device.channel) {
          deviceChannels[device.serial] = device.channel
        }
      })
    })
    .catch(function() {})
  ...
  return db.run(r.table('automationExplorerRunDevices').insert(deviceRows))
    .then(function() {
      var automationOwner = {
        email: run.createdByEmail || 'automation'
      , name: run.createdByName || 'automation'
      , group: 'automation'
      }
      return Promise.map(deviceRows, function(row) {
        // 运行前直接标记 DB，作为 GroupMessage 流程的兜底/加速
        dbapi.setDeviceOwner(row.serial, automationOwner).catch(function() {})
        dbapi.setDeviceUsage(row.serial, 'automation').catch(function() {})
        function releaseDevice() {
          // 通过 Wire 消息释放设备，让 WebSocket 广播 device.change（owner=null）
          if (push && ownerMsg) {
            var channel = deviceChannels[row.serial]
            if (channel) {
              push.send([
                channel
              , wireutil.envelope(new wire.UngroupMessage(
                  wireutil.toDeviceRequirements({
                    serial: { value: row.serial, match: 'exact' }
                  })
                , 'bulk_task_release'
                ))
              ])
          ...
          // 兜底：直接清 DB
          dbapi.unsetDeviceOwner(row.serial).catch(function() {})
          dbapi.unsetDeviceUsage(row.serial).catch(function() {})
        }
        ...
        , initialMemory: run.initialMemory || null
          ...
          .finally(function() { releaseDevice() })
          .catch(function(err) {
            var endedAt = now()
