...
function addGroupDevices(req, res) {
  ...
  function addGroupDevice(group, serial) {
    const lock = {}
    const lockDevice = group.class === apiutil.ONCE ?
      dbapi.lockDeviceByOrigin :
      dbapi.lockBookableDevice

    return lockDevice(req.user.groups.subscribed, serial).then(function(stats) {
      if (!stats.replaced) {
...
function getGroupDevices(req, res) {
  ...
  dbapi.getUserGroup(req.user.email, id).then(function(group) {
    ...
    if (bookable) {
      if (apiutil.isOriginGroup(group.class)) {
        dbapi.loadDevices(req.user.groups.subscribed).then(function(devices) {
          const deviceList = []
          devices.forEach(function(device) {
            if (group.devices.indexOf(device.serial) < 0) {
      ...
      // For 'once' groups, return ALL origin-accessible devices (including standard class).
      // Note: do NOT filter out devices already in the group here!
      // The frontend groupObjectsFilter needs ALL devices (including group members) in
      // availableDevicesBySerial to display the group-devices table.
      // The availableObjectsFilter on the 'Available devices' panel handles the exclusion.
