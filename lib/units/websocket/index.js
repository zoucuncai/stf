...
module.exports = function(options) {
  ...
  io.on('connection', function(socket) {
    ...
    var messageListener = wirerouter()
      ...
      .on(wire.PerformanceEvent, function(channel, message) {
        var serial = message.serial
        delete message.serial
        socket.emit('device.change', {
          important: false
        , data: {
            serial: serial
          , performance: {
              cpu: message.cpuPercent
            , mem: message.memPercent
            , memTotal: message.memTotal
            , memUsed: message.memUsed
            }
          }
        })
      })
    ...
    new Promise(function(resolve) {
      ...
      socket.on('disconnect', resolve)
        ...
        // Admin-only: update the device "responsible" (归属人) field.
        .on('device.responsible', function(data) {
          if (user.privilege !== 'admin') {
            return Promise.resolve()
          }

          const serial = data.serial
          const responsibleEmail = data.responsibleEmail

          // Allow clearing by sending empty/null.
          if (!responsibleEmail) {
            return dbapi.setDeviceResponsible(serial, null)
              .then(function() {
                return dbapi.loadDevice(user.groups.subscribed, serial)
              })
              .then(function(cursor) {
                if (cursor) {
                  cursor.next(function(err, device) {
          ...
          return dbapi.loadUser(responsibleEmail)
            .then(function(responsibleUser) {
              const responsible = responsibleUser ?
                {email: responsibleUser.email, name: responsibleUser.name} :
                null
              return dbapi.setDeviceResponsible(serial, responsible)
            })
            .then(function() {
              return dbapi.loadDevice(user.groups.subscribed, serial)
            })
        ...
        .on('device.install', function(channel, responseChannel, data) {
          ...
          , wireutil.transaction(
            ...
            , new wire.InstallMessage(
              ...
              , data.persist_after_session === true
              )
            )
          ])
        })
        ...
        .on('fs.push', function(channel, responseChannel, data) {
          ...
          , wireutil.transaction(
              responseChannel
            , new wire.FileSystemPushMessage(
                data.href
              , data.target
              , data.filename || ''
              )
