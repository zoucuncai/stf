...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push) {
    ...
    router.on(wire.InstallMessage, function(channel, message) {
      ...
      var persistAfterSession =
        message.persist_after_session === true ||
        message.persistAfterSession === true

      log.info(
        'Installing package "%s" from "%s"%s'
      , pkg
      , message.href
      , persistAfterSession ? ' (persist after session)' : ''
      )
      ...
      pushApp()
        ...
        .then(function() {
          if (persistAfterSession) {
            persistentInstalls.markPersistent(pkg)
            log.info('Package "%s" will not be removed on session cleanup', pkg)
          }
    ...
    router.on(wire.UninstallMessage, function(channel, message) {
      ...
      adb.uninstall(options.serial, message.packageName)
        ...
        .catch(function(err) {
          log.error(
            'Uninstallation failed for "%s" on %s: %s'
          , message.packageName
          , options.serial
          , err.stack
          )
          ...
          push.send([
            channel
          , reply.fail(detail || 'UNINSTALL_FAILED')
          ])
        })
    })
  })
